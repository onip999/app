...
export default function App(){
 ...
 async function loadProducts(){
  ...
  const {data,error:fetchError}=await supabase.from('products').select(`id,brand,name,category,description,image_url,offers(id,price,shipping_cost,availability,product_url,updated_at,stores(id,name,website_url,return_days,warranty_months,shipping_notes))`).order('brand',{ascending:true})
 ...
 if(selectedProduct){
  const offers=selectedProduct.offers||[],scoredOffers=offers.map(offer=>({offer,score:convenienceScore(offer,offers)})),recommendedId=[...scoredOffers].sort((a,b)=>b.score-a.score)[0]?.offer?.id,insight=priceInsight(selectedProduct)
