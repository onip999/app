...
-- Una riga per ogni rilevazione del prezzo di una singola offerta.
-- Questa tabella permetterà di calcolare media 30/90 giorni, minimo storico e trend.
...
create policy public_insert_clicks on public.clicks for insert with check (true);

-- Sicurezza:
-- il browser può LEGGERE lo storico prezzi, ma non può scriverlo.
-- Le rilevazioni future dovranno essere inserite da un processo server/autorizzato.
-- Non esiste una policy SELECT su clicks: i visitatori non possono leggere le statistiche dei click.
